...
class qa_dataset(data.Dataset):
    def __init__(self):
        '''
        :param dataset_type: 'train'
        '''
        super(qa_dataset, self).__init__()
        
        self.feature_dir = "/net/kdinxidk03/opt/NFS/75y/data/qa/features_cpg_biolinkbert/"
        self.qid_list_dir = "/net/kdinxidk03/opt/NFS/75y/data/qa/qid_list_dir_cpg_biolinkbert/"

        self.domain_names = configs.in_domain_names
...
#                                         for qid in tqdm(open(self.qid_list_dir + \
#                                                              f"{name}_qid_list.txt", "r").readlines())]
#             len_+=len(self.qid_list_dict[name])
        
#         self.data_len = int(len_/3) #configs.num_samples_per_epoch
#         while self.data_len%configs.per_gpu_train_batch_size!=0:
#             self.data_len+=1
...
#                torch.tensor([is_same_domain], dtype=torch.float)
...
def qa_collate(samples):
    ...
    # cls_index0_batch = torch.cat(cls_index0_batch)
    # p_mask0_batch = torch.cat(p_mask0_batch)
    # is_impossible0_batch = torch.cat(is_impossible0_batch)
    ...
    # cls_index1_batch = torch.cat(cls_index1_batch)
    # p_mask1_batch = torch.cat(p_mask1_batch)
    # is_impossible1_batch = torch.cat(is_impossible1_batch)
    ...
    # cls_index2_batch = torch.cat(cls_index2_batch)
    # p_mask2_batch = torch.cat(p_mask2_batch)
    # is_impossible2_batch = torch.cat(is_impossible2_batch)
    start_positions2_batch = torch.cat(start_positions2_batch)
    end_positions2_batch = torch.cat(end_positions2_batch)

#     is_same_domain_batch = torch.cat(is_same_domain_batch)
