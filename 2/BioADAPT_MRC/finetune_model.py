...
from BioADAPT_MRC.load_dataset import load_and_cache_examples
...
#             print(len(outputs), outputs[0].shape, outputs[1].shape)
    ...
    assert list(em.keys()) == list(f1.keys())
...
def main():
    ...
    parser.add_argument("--logging_steps", type=int, default=500_000, help="Log every X updates steps.")
    parser.add_argument("--n_gpu", type=int, default=1)
    parser.add_argument("--save_steps", type=int, default=500_000, help="Save checkpoint every X updates steps.")
    parser.add_argument(
    ...
    parser.add_argument(
        "--overwrite_output_model_dir", action="store_true", help="Overwrite the content of the output directory"
    )
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model = AutoModelForQuestionAnswering.from_pretrained(args.output_model_dir)  # , force_download=True)
